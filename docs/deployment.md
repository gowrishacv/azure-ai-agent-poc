...
The application and indexing script use the account's Azure OpenAI-compatible
v1 endpoint (`https://<account>.openai.azure.com/openai/v1/`) for chat and
embeddings, with Microsoft Entra tokens scoped to
`https://ai.azure.com/.default`. New model deployments can take several minutes
to become available, so the indexing script retries only
`DeploymentNotFound` responses for a bounded period.
